...
7. **Multiple matches with the same trigger.** Espanso uses the most specific/most recently loaded match when triggers collide across files — if behavior seems to be "the wrong" version of a trigger, search all match files for duplicate trigger strings.
